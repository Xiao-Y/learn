...
public class ExcelExporterKit {
    ...
    public static ExcelExporterKit getInstance() {
        return SpringContextUtil.getBean(ExcelExporterKit.class);
    }
...
//                String filePath = System.getProperty("java.io.tmpdir") + "/" + taskId + ".xlsx";
    ...
    private void createHeader(Workbook workbook, Sheet sheet, List<Field> fields) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            // 设置下拉列表
            if (!ExcelColumn.DictType.NULL.equals(column.dictType())) {
                List<String> options = dictService.getDictLabels(column.dictCode(), column.dictType());
                if (!options.isEmpty()) {
            ...
            for (int j = 0; j < fields.size(); j++) {
                ...
                try {
                    ...
                    if (value != null) {
                        ...
                        // 1. 首先检查是否需要字典转换
                        if (!ExcelColumn.DictType.NULL.equals(column.dictType())) {
                            String dictCode = Optional.ofNullable(column.dictCode()).orElse(field.getName());
                            String dictValue = value.toString();
                            String label = dictService.getLabelByValue(dictCode, dictValue, column.dictType());
