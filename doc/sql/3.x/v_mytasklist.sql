CREATE
OR REPLACE  VIEW `v_mytasklist` AS
SELECT `rt`.`EXECUTION_ID_`     AS `executionId`,
...
FROM (`sys_apply_info` `sai` LEFT JOIN `ACT_RU_TASK` `rt` ON ((`sai`.`proc_inst_id` = `rt`.`PROC_INST_ID_`)))
WHERE ((`sai`.`is_end` = FALSE) AND (`rt`.`ASSIGNEE_` IS NOT NULL))
UNION ALL
SELECT `rt`.`EXECUTION_ID_`     AS `executionId`,
...
WHERE ((`sai`.`is_end` = FALSE) AND isnull(`rt`.`ASSIGNEE_`) AND (`I`.`TYPE_` = 'candidate'));